...
@ccclass
export default class FriendListView extends cc.Component {
    ...
    private checkNeedUpdate(): number {
        ...
        let scroll;
        if(this.horizontal){
            scroll = this.content.parent.width * (this.content.parent.anchorX - 1) - this.content.x;
        }else{
            (this.content.y - this.content.parent.height * this.content.parent.anchorY);
        }
        let itemStartIndex = Math.floor(scroll / ((this.horizontal ? this._itemWidth : this._itemHeight) + this.spacing));
        // console.log(scroll,this.content.x,this.content.parent.width,this.content.parent.anchorX,itemStartIndex,"=====");
